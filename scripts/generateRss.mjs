...
async function getAllPublications(type, locale) {
  ...
  const creativityDirectory = path.join(
    process.cwd(),
    `_content/creativity/${locale}`,
  )
  ...
  const publicationsWithHtml = await Promise.all(
    publications.map(async (publication) => {
      const result = await remark()
        .use(imgLinks, {
          absolutePath: `https://alexandr-sidorenko.me/${locale}`,
        })
        .use(html)
        .process(publication.content)
      const htmlContent = result.toString()
      return { ...publication, content: htmlContent }
    }),
  )

  return publicationsWithHtml.sort((post1, post2) =>
...
async function generateRss(type, locale) {
  ...
  const author = {
    ...
    link: isPost
      ? 'https://twitter.com/batyshkaLenin/'
      : 'https://vk.com/better_not_be_born',
  }
  ...
  const feed = new Feed({
    ...
    description: isPost
      ? locales[locale]['BLOG_TITLE']
      : locales[locale]['CREATIVITY_TITLE'],
    id: siteURL,
    link: siteURL,
    ...
    author,
    copyright: `${
      locales[locale]['RIGHTS']
    } 2020-${new Date().getFullYear()}, ${locales[locale]['FULL_NAME']}`,
    language: locale,
  })
  ...
  const rssDir = `./public/rss/${locale}/${path}`
  await fs.promises.mkdir(rssDir, { recursive: true })
  await Promise.all([
    fs.promises.writeFile(
      `${rssDir}/feed.xml`,
      feed.rss2().replaceAll('<item>', '<item turbo="true">'),
    ),
    fs.promises.writeFile(`${rssDir}/feed.json`, feed.json1()),
  ])
}
...
async function generateAllRSS() {
  ...
  for (let i = 0; i < types.length; i++) {
    const type = types[i]
    try {
      await Promise.all(['ru', 'en'].map((locale) => generateRss(type, locale)))
...
function parseTimeline(txt) {
  ...
  const timeline = splittedTimeline
    .map((i) => i.split('\t'))
    .filter((i) => i.length === 2)
...
async function generateJournal() {
  const currentTimelineText = fs
    .readFileSync(path.join(process.cwd(), 'public/twtxt.txt'))
    .toString()
  const parsedTimeline = parseTimeline(currentTimelineText).filter(
  ...
  let journalText =
    '<html lang="ru"><head><title>Журнал Александра Сидоренко</title></head><body><h1>Журнал Александра Сидоренко</h1>'
  Object.keys(groups).forEach((date) => {
    journalText += `<article><h2>${date}</h2>${groups[date]
      .map((item) => `<p>${encode(item)}</p>`)
      .join('')}</article>`
  })
