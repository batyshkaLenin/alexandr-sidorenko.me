...
The search script lives inside this skill's own directory, not the project directory. Always invoke it by its full path from the repository root. Cursor and Codex do not set `CLAUDE_PLUGIN_ROOT`; use the project-relative path below. Prefer `python3`.
