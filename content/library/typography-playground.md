...
Формулы набираются в LaTeX и превращаются в MathML на сборке (§29): инлайновая
формула — между `\(` и `\)`, выключная — между `$$` и `$$`. Клиентского
скрипта для их появления нет.

Инлайновая формула: \( f(x) = x^2 + 2x + 1 \) внутри строки текста.
...
Нумерованное уравнение:

$$
f(x) = \sum_{i=1}^{n} a_i \varphi_i(x) \tag{12}
$$
...
Очень широкое выражение — оно должно прокручиваться внутри своего контейнера, а
не растягивать страницу:

$$
\Phi(x) = \int_{-\infty}^{x} \frac{1}{\sqrt{2\pi}} e^{-t^2/2}\,dt + \sum_{k=0}^{50} \frac{(-1)^k x^{2k+1}}{(2k+1)!} + \prod_{j=1}^{30} \left(1 + \frac{x}{j}\right)
$$
